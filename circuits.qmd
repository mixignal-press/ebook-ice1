...
Circuit behavior is governed by two constraints: (1) Kirchhoff’s laws and (2) $i–v$ relationships. These constraints expressed in terms of voltages and currents lead to numerous simultaneous equations. For instance, a six-device circuit requires solving 12 equations with 12 unknowns. Simplifying these equations is ideal, despite available computation tools.

Using node voltages as circuit variables reduces the number of simultaneous equations. Define node voltages by selecting a reference node, typically ground, and measuring the voltage between the remaining nodes and this reference.

Use node voltage analysis to express element voltages without explicitly writing KVL equations.

![Circuit for demonstrating node-voltage analysis](figures/fig-circuits-nodev.png){#fig-circuits-nodev}

The circuit in @fig-circuits-nodev illustrates node-voltage equation formulation, identifying a reference node (ground), four currents ($i_0$, $i_1$, $i_2$, $i_3$), and two node voltages ($v_A$, $v_B$).
...
In summary, creating node-voltage equations requires four steps:

- **STEP 1**: Select a reference node. Assign a voltage to each of the other $N– 1$ nodes and determine a current for each circuit element.
- **STEP 2**: Express the KCL connection constraints through the currents of the elements at the $N-1$ non-reference nodes.
- **STEP 3**: Utilize the $i–v$ relationships and the principles of nodal analysis to express the currents traversing elements concerning node voltages.
- **STEP 4**: Incorporate the element constraints from step 3 into the KCL constraints from step 2, then format the resulting N–1 equations into standard form.
...
::: {.content-hidden}
FIXME: Add python and ngspice example
:::
