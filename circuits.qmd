...
![A linear circuit with resistors and both dependent and independent sources is connected to a linear or non-linear load.](fig-circuits-linckt.png){#fig-circuits-linckt}
...
![Thevenin Equivalent: (a) Disconnect the load and measure the open-circuit voltage. (b) Zero all independent sources to find the equivalent resistance. (c) The Thévenin circuit combines the open-circuit voltage in series with the equivalent resistance.](fig-circuits-thevenin.png){#fig-circuits-thevenin}
